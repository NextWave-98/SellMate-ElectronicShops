...
import type { SubscriptionPlan, UpdateSubscriptionPlanDTO, SubscriptionPlanType } from '../../../types/subscription.types';
import toast from 'react-hot-toast';

interface EditPlanModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (id: string, data: UpdateSubscriptionPlanDTO) => Promise<void>;
  plan: SubscriptionPlan | null;
  isLoading: boolean;
}

export default function EditPlanModal({ isOpen, onClose, onSubmit, plan, isLoading }: EditPlanModalProps) {
  ...
  useEffect(() => {
    if (plan) {
      setFormData({
        ...
        semiAnnualPrice: String(plan.semiAnnualPrice) || '',
        ...
        unlimitedStorage: plan.unlimitedStorage || false,
        posEnabled: plan.posEnabled || false,
        onlineSalesEnabled: plan.onlineSalesEnabled || false,
        courierIntegrationEnabled: plan.courierIntegrationEnabled || false,
        advancedReporting: plan.advancedReporting || false,
        apiAccess: plan.apiAccess || false,
        customBranding: plan.customBranding || false,
        multiCurrency: plan.multiCurrency || false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await onSubmit(plan.id, updateData);
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : 'Failed to update plan';
        ...
        <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="sticky top-0 flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Edit Subscription Plan</h2>
