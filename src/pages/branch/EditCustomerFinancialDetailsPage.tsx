...
export default function EditCustomerFinancialDetailsPage() {
    ...
    const { fetchData: updateFinancialDetails } = useFetch('/installments/financial-details');
        ...
        try {
            ...
            if (response?.success && response?.data) {
                ...
                setFormData({
                    ...
                    nationalIdIssueDate: details.nationalIdIssueDate ? new Date(details.nationalIdIssueDate).toISOString().split('T')[0] : '',
                    nationalIdExpiryDate: details.nationalIdExpiryDate ? new Date(details.nationalIdExpiryDate).toISOString().split('T')[0] : '',
                ...
                toast.error('Financial details not found');
                navigate(`../customers/${customerId}/financial-details/add`);
            }
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setLoading(true);
            const response = await updateFinancialDetails({
                method: 'PUT',
                endpoint: `/${customerId}`,
                data: {
                    ...formData,
    ...
    if (loadingCustomer || loadingDetails) {
        return (
    ...
    return (
        <div className="mx-auto space-y-6">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-bold text-gray-900">Edit Financial Details</h1>
                    <p className="text-gray-600 mt-1">Update financial information for {customer.name}</p>
                </div>
            </div>
